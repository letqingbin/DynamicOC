...
@property (nonatomic,strong) ASTBlockNode *node;
...
void JPBlockInterpreter(ffi_cif *cif, void *ret, void **args, void *userdata)
{
    ...
    [blockObj.node.context pushDefinedContext:blockObj.blockContext];
    ...
    for (int i = 1; i < blockObj.signature.argumentTypes.count; i ++) {
        ...
        ASTVariable* var = blockObj.node.arguments[i-1];
        NSString* key = var.name;
        var.value = param;

		[blockObj.node.context pushValue:var forKey:key];
    }
    
    ASTNode* scopeNode = blockObj.node.allChilds[2];
    ASTVariable* var = [scopeNode execute];
    id result = var.value;
    
    [blockObj.node.context popContext];
...
+ (instancetype)blockWrapperWithNode:(ASTBlockNode *)node
{
    ...
    wrapper.node = node;
	wrapper.blockContext = node.blockContext;
    return wrapper;
}
...
+ (id)excuteBlock:(id)block withParams:(NSArray *)params
{
    ...
    return [ASTCallMethod invokeWithInvocation:invocation argments:params];
}
